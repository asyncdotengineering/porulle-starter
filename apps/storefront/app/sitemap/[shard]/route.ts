...
import { getSitemapPage, type SitemapType } from "@/lib/commerce/operations/sitemap";
...
async function renderShard(
  ...
  const { items } = await getSitemapPage(type, page);
...
export async function GET(
  ...
  const type: SitemapType = match[1] === "products" ? "PRODUCT" : "COLLECTION";
