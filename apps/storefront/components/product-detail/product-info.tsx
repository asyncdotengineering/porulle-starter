...
function ProductInfoOptions({
  ...
  // Hide the synthetic "Title / Default Title" option a variant-less product carries.
  const isDefaultTitleOption = (opt: ProductOption) =>
  ...
  const renderable = options.filter((opt) => !isDefaultTitleOption(opt));
